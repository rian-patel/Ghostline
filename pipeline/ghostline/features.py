...
def _corner_metrics(channels, i0, i1):
    """(apex_speed km/h, apex_distance m, brake_point m-before-apex|None) for
    one driver in one corner window, or None if the window is degenerate."""
    speed = channels["speed"]
    brake = channels["brake"]
    dist = channels["distance"]
    n = len(speed)
    ...
    for k in range(i0, apex + 1):
        if brake[k]:
            brake_point = apex_dist - float(dist[k])
            break
    return apex_speed, apex_dist, brake_point


def _corner_label(corner):
    return f"Turn {corner['number']}{corner.get('letter', '')}".strip()


def generate_findings(drivers, pole_code, corners, max_findings=12,
                      min_gain_s=0.03, approach_m=120.0, exit_m=80.0):
    """Plain-English, number-filled findings comparing each driver to pole.

    Every finding is derived only from already-computed channels (delta, speed,
    brake) so the AI debrief narrates real numbers it never had to invent. For
    each driver/corner we read the time gained or lost across a band around the
    apex (approach_m before to exit_m after) — tight enough to isolate the
    corner from the straights either side — and attach the dominant mechanical
    cause (braking later/earlier, or higher/lower apex speed) versus pole. The
    most significant findings across the field are returned, biggest swing
    first.
    """
    codes = list(drivers)
    if len(corners) < 1 or len(codes) < 2:
        return []

    pole = drivers[pole_code]["channels"]
    n = len(pole["speed"])

    def band(apex_dist):
        i0 = max(0, min(n - 1, int(round((apex_dist - approach_m) / STEP_M))))
        i1 = max(0, min(n - 1, int(round((apex_dist + exit_m) / STEP_M))))
        return i0, i1

    candidates = []  # (magnitude_s, string)
    for code in codes:
        if code == pole_code:
            continue
        ch = drivers[code]["channels"]
        d = ch["delta"]
        for corner in corners:
            i0, i1 = band(corner["distance"])
            if i1 <= i0:
                continue
            j0 = min(i0, len(d) - 1)
            j1 = min(i1, len(d) - 1)
            change = float(d[j1] - d[j0])  # +ve = lost time to pole here
            if abs(change) < min_gain_s:
                continue

            pm = _corner_metrics(pole, i0, i1)
            dm = _corner_metrics(ch, i0, i1)
            if pm is None or dm is None:
                continue
            p_apex, _, p_bp = pm
            v_apex, _, v_bp = dm

            # Dominant cause: braking-point difference, else apex-speed.
            reason = ""
            if p_bp is not None and v_bp is not None:
                later = p_bp - v_bp  # +ve => brakes later (closer to apex)
                if abs(later) >= 3.0:
                    reason = (f"braking {abs(later):.0f}m "
                              f"{'later' if later > 0 else 'earlier'}")
            if not reason:
                dv = v_apex - p_apex
                if abs(dv) >= 2.0:
                    reason = (f"carrying {abs(dv):.0f} km/h "
                              f"{'more' if dv > 0 else 'less'} apex speed")

            verb = "gains" if change < 0 else "loses"
            label = _corner_label(corner)
            text = f"{code} {verb} {abs(change):.2f}s into {label}"
            if reason:
                text += f" by {reason} than {pole_code}" if change < 0 \
                    else f", {reason} than {pole_code}"
            text += "."
            candidates.append((abs(change), text))

    candidates.sort(key=lambda c: c[0], reverse=True)
    return [text for _, text in candidates[:max_findings]]
