...
`npm run dev` reloads automatically as files change. `web/public/sessions/` is copied verbatim into `web/dist/sessions/` by the build, so any session you bake is included.

## Deploy to Vercel

The app is a static Vite site; `vercel.json` sets the build command and `web/dist` as the output. To deploy with the Vercel CLI from the repo root:
...
For continuous deploys, connect the GitHub repository in the Vercel dashboard (Project → Settings → Git). Every `git push` to the default branch then rebuilds and updates the live site automatically.

## FastF1 accuracy caveats

The public telemetry feed has real limits, and Ghostline is built around them rather than hiding them:

- Position data arrives at only ~4–5 Hz and is jittery; anything smoother is interpolation. X/Y are smoothed with a Savitzky–Golay filter *before* differentiating, and dynamics are computed on raw single-lap data *before* resampling onto the 5 m grid.
- **Lateral g** (`speed² · curvature / 9.81`) comes out realistic (~4–6 g in fast corners) and is trustworthy. **Longitudinal g** (`d(speed)/dt / 9.81`) is approximate — per FastF1's own maintainer, hard to compute reliably from this feed — and is labelled as such in the app.
- Overlaying two different laps carries roughly ±10 m of position uncertainty, so the track map is for visualization; timing math anchors to the reliable speed and time channels.
- 2018+ seasons only. The 2026 public feed has no DRS, active-aero, or energy-deployment data.

## Credits

Built on [FastF1](https://github.com/theOehrly/Fast-F1). This is an unofficial project and is not associated with, endorsed by, or affiliated with Formula 1, the FIA, or any team.
